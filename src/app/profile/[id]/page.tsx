...
useEffect(() => {
  async function fetchSubscriptionData() {
    try {
      ...
      // Get the subscription index from the URL
      ...
      // Get user data to access the metadata URIs
      ...
      // Find the specific metadata URI for this subscription
      const metadataUri = metadataUris[Number(id)] || metadataUris[0];
      
      if (!metadataUri) {
        throw new Error('No metadata URI found for this subscription');
      }
      ...
      if (existingRecord) {
        ...
        const { data, error } = await supabase
          ...
          .update({
            ...
            payment_amount: subscription.paymentAmount,
                ...
                <div className="space-y-6 text-white">
                  {loading ? (
                    ...
                    <div className="border rounded-md">
                      <Table >
                        <TableHeader>
                          <TableRow className="bg-gradient-to-r from-indigo-600 to-cyan-700 w-full text-white">
                            ...
                            <div key={`${year}-${month.value}`} className="border rounded-md overflow-hidden">
                              <div className="bg-muted p-3 flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-4">
                                  ...
                                  <span className="font-medium">${totalAmount.toFixed(2)} / ${subscription.paymentAmount * monthlyPayments.length}</span>
                                </div>
                              </div>
                                ...
                                <TableBody>
                                  {monthlyPayments.map((payment) => (
                                    <TableRow key={payment.id} className="text-white">
                                      <TableCell>{getUserName(payment.shared_user_id)}</TableCell>
